...
remUNI	=	function(IN)	IN[which(lapply(lapply(IN, unique), length) != 1)]
...
CAPTION	=	c(CPUname, COOLERname)
CAPTION	=	c(CAPTION,	ifelse(is.null(PULSE), TESTname, paste0(TESTname, " (", PULSE, " s)"))	)
...
FREQspec_line	=	function(FREQ	=	FREQspec)	{
	...
	FREQdata	=	list(
		...
		TEXT	=	FREQspec,
		ECDF	=	round2(ecdf(dataALL[dataALL$Period == TESTname, ]$Frequency)(FREQspec))
		)
...
message("Frequency")
customSave("Frequency", 		plot = graphFREQ(FREQ.COEF*0.5),		height	=	2 * length(unique(dataALL$Thread)))
message("Core Power")
customSave("Core Power",		plot = graphPOWER(FREQ.COEF*0.5),	height	=	2 * length(unique(dataALL$Core)))
